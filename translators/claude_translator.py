...
6. 配列内の隣り合う要素は、PDFの段組み(2カラムやページの都合)によって
   1つの文・段落が分割されたものであることがあります。文が句点や読点なしで
   終わっている要素の次の要素は、その続きである可能性を考慮し、同じ動詞の
   活用語尾を続けて繰り返す(例:「〜を示し、」の直後に「〜を示し、」)など
   不自然な言葉の重複を避け、自然につながる訳文にしてください。
   下記の文脈情報に「直前のブロックの訳文」が示されている場合は、それとの
   接続も自然になるようにしてください。
"""
